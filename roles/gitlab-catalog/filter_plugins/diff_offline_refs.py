def diff_offline_branches(source_branches, destination_branches):
  to_push = []
  to_delete = []
...
def diff_offline_tags(source_tags, destination_tags):
  ...
  for s in source_tags:
    ...
    for d in destination_tags:
      if s['name'] == d['name'] and s['commit']['id'] != d['target']:
        to_push.append(s['name'])
        continue
...
class FilterModule(object):  
    def filters(self):    
        return {
            'diff_offline_branches': diff_offline_branches,
            'diff_offline_tags': diff_offline_tags,
        }
